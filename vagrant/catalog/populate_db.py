...
for x in range(1, 10):
    if(x % 2) == 0:
        category = Category(user_id=User0.id, name='Category Name ' + str(x),
                            description=description)
    else:
        category = Category(user_id=User1.id, name='Category Name' + str(x),
                            description=description)
    ...
    items = []
    for i in range(1, 5):
        items.append(Item(name='Item Name '+str(i),
                     description="Sample Item description item database id" +
                                 "number for testing is " + str((x-1)*5 + i),
